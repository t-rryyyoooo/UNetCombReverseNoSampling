...
echo -n "Is json file name createFeatureMapV3KFold.json?[y/n]:"
read which
...
# Specify json file.
if [ $which = "y" ];then
 JSON_NAME="createFeatureMapV3KFold.json"
else
...
readonly PADDING_SIZE=$(cat ${JSON_FILE} | jq -r ".padding_size")
...
do
    ...
    do
     ...
     python3 createFeatureMapV2.py ${image_path} ${modelweight_path} ${save_path} --image_patch_size ${IMAGE_PATCH_SIZE} --label_patch_size ${LABEL_PATCH_SIZE} --padding_size ${PADDING_SIZE} ${mask} ${is_label}
