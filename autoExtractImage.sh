...
readonly SAVE_TYPE=$(cat ${JSON_FILE} | jq -r ".save_type")
...
do
 ...
 echo "SAVE_TYPE:${SAVE_TYPE}"

 python3 extractImage.py ${image} ${label} ${save} --mask_path ${mask} --image_patch_size ${IMAGE_PATCH_SIZE} --label_patch_size ${LABEL_PATCH_SIZE} --overlap ${OVERLAP} --save_type ${SAVE_TYPE}
