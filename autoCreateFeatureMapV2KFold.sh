...
echo -n "Is json file name createFeatureMapV2KFold.json?[y/n]:"
read which
...
# Specify json file.
if [ $which = "y" ];then
 JSON_NAME="createFeatureMapV2KFold.json"
else
...
readonly PADDING_SIZE_IN_MODEL=$(cat ${JSON_FILE} | jq -r ".padding_size_in_model")
...
do
    ...
    do
     ...
     echo "PADDING_SIZE_IN_MODEL:${PADDING_SIZE_IN_MODEL}"
     echo "MASK_NAME:${MASK_NAME}"
     echo "is_label:${IS_LABEL}"

     python3 createFeatureMapV2.py ${image_path} ${modelweight_path} ${save_path} --image_patch_size ${IMAGE_PATCH_SIZE} --label_patch_size ${LABEL_PATCH_SIZE} --padding_size_in_model ${PADDING_SIZE_IN_MODEL} ${mask} ${is_label}
