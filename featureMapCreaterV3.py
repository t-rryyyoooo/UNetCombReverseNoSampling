...
class FeatureMapCreater():
    def __init__(self, image, model, image_patch_size, label_patch_size, mask=None, padding_size=[10, 10, 10], is_label=False):
        self.image = image
        self.model = model
    ...
    def execute(self):
        """ To make feature map correctly, add self.padding_size."""
        padded_image_patch_size = self.image_patch_size + self.padding_size * 2
        ...
        with tqdm(total=len(image_array_list), desc="Making feature maps..", ncols=60) as pbar:
            for image_array in image_array_list:
                ...
                lower_crop_size = [0] + self.padding_size.tolist()
                upper_crop_size = [0] + self.padding_size.tolist()
